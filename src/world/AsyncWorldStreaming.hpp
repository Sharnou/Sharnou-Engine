...
class AsyncWorldStreamer {
    ...
    CellId cell(float x,float z) const noexcept { return {static_cast<std::int32_t>(x/cellSize_),static_cast<std::int32_t>(z/cellSize_)}; }
    void enqueue(CellId id) {
        ...
        jobs_.push([this,id]{
            { std::scoped_lock lock(mutex_); auto it=records_.find(id); if(it==records_.end())return; it->second.state=State::Loading; }
            std::error_code ec; const auto path=records_.at(id).manifest.asset; const bool exists=std::filesystem::exists(path,ec);
            std::scoped_lock lock(mutex_); auto it=records_.find(id); if(it==records_.end())return; it->second.state=exists?State::Resident:State::Resident; ++it->second.manifest.generation;
        });
        wake_.notify_one();
    }
public:
    ...
    void configure(float cellSize,std::int32_t radius){cellSize_=cellSize;radius_=radius<0?0:radius;}
    void update(float x,float z){const CellId c=cell(x,z);std::scoped_lock lock(mutex_);for(std::int32_t dz=-radius_;dz<=radius_;++dz)for(std::int32_t dx=-radius_;dx<=radius_;++dx)enqueue({c.x+dx,c.z+dz});for(auto&[id,r]:records_)if(std::abs(id.x-c.x)>radius_+1||std::abs(id.z-c.z)>radius_+1)r.state=State::Unloaded;}
    std::size_t resident() const {std::scoped_lock lock(mutex_);std::size_t n=0;for(auto&[id,r]:records_)n+=r.state==State::Resident;return n;}
